-- ============================================================
-- 000 — INITIAL SCHEMA (frozen baseline / migration zero)
-- ============================================================
--
-- This is the historical starting point of the database: the base tables as
-- they existed BEFORE row-level multitenancy (001+). It is single-tenant and
-- deliberately does NOT reflect the current schema — later migrations add
-- tenant_id, RLS, the identity/issues/lessons tables, per-tenant keys, etc.
--
--   *** DO NOT EDIT THIS FILE. ***  It is frozen history. Every schema change is
--   a NEW numbered migration in this directory. There is no separate
--   "from-scratch end-state" file to keep in sync anymore — the current schema
--   IS the ordered migrations, replayed.
--
-- To build a database from zero (disaster recovery / a fresh clone), run every
-- file in scripts/migrations/ in numeric order: 000, 001, 002, …
-- For a human-readable description of the CURRENT state, see
-- docs/DATABASE_ARCHITECTURE.md — not this file.
-- ============================================================
