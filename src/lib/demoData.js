  ...
  T.theses = [
    {
      ...
      assumptions: JSON.stringify([{ type: 'text', value: 'Debt issuance normalizes with the 2026-2028 maturity wall; MA decays no faster than ~5%/yr in the bear case; buybacks retire ~1.5%/yr; no structural change to the NRSRO regime.' }]),
    ...
    },
    {
      ...
      assumptions: JSON.stringify([{ type: 'text', value: 'Branded TPV share stabilizes only if checkout latency + Fastlane adoption actually move conversion; assume unbranded take rate never exceeds 120bps.' }]),
    ...
    },
    {
      ...
      assumptions: JSON.stringify([{ type: 'text', value: 'Mortgage score pricing holds (no freeze); VantageScore adoption limited to non-conforming; software segment stays GDP-plus and is effectively a free option.' }]),
    ...
    },
    {
      ...
      assumptions: JSON.stringify([{ type: 'text', value: 'USD revenue CAGR 25%+ through 2028; credit book grows <= TPV growth with loss rates capped near current levels; no capital raise.' }]),
    ...
    },
    {
      ...
      assumptions: JSON.stringify([{ type: 'text', value: 'Hardware revenue GDP-ish, services 11-12% CAGR, gross margin drifts up on mix, buyback ~3%/yr continues, no material China disruption.' }]),
    ...
    },
    {
      ...
      assumptions: JSON.stringify([{ type: 'text', value: 'Azure 25%+ through 2027; Copilot attach reaching 15% of eligible seats; margins dip <=150bps on AI capex then recover.' }]),
    ...
    },
    {
      ...
      assumptions: JSON.stringify([{ type: 'text', value: 'Payments volume grows nominal-GDP-plus; take rate flat (regulatory pressure offset by value-added services); buyback ~3%/yr.' }]),
    ...
    },
    {
      ...
      assumptions: JSON.stringify([{ type: 'text', value: 'Leading-edge share holds >85%; gross margin 55%+ through the capex cycle; Arizona/Kumamoto fabs dilute margin <200bps; no kinetic Taiwan event (unhedgeable - sized accordingly).' }]),
    ...
    },
    {
      ...
      assumptions: JSON.stringify([{ type: 'text', value: 'Revenue ~14% CAGR, gross margin through 32%, opex growth capped below revenue growth, MAU growth mid-single-digit in developed + strong EM mix.' }]),
    ...
    },
    {
      ...
      assumptions: JSON.stringify([{ type: 'text', value: 'AI networking + XPU revenue CAGR 30%+ through 2028; VMware retention holds through the pricing migration; dividend grows with FCF; leverage glides back under 2x.' }]),
